...
const AdminDashboard = () => {
  ...
  if (loading) return <div style={{textAlign: 'center', marginTop: '50px', color: '#0056b3'}}>Loading Admin Portal...</div>;
  ...
  return (
    <div className="container fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '25px' }}>
        {filteredComplaints.length === 0 ? <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '40px', color: '#999' }}>No reports found.</div> : filteredComplaints.map(c => (
          <div key={c.id} className="gov-card" style={{ padding: '0' }}>
            ...
            <div style={{ padding: '15px' }}>
              ...
              <p style={{ color: '#666', fontSize: '0.9rem', marginBottom: '15px' }}>{c.description}</p>
              
              {/* ASSIGNMENT AREA */}
              <div style={{ background: '#f1f1f1', padding: '10px', borderRadius: '4px' }}>
                <label style={{ fontSize: '0.8rem', fontWeight: 'bold', color: '#555' }}>
                  {c.assigned_to ? '👤 Worker Assigned:' : '⚠️ Assign Task To:'}
                </label>
                <div style={{ display: 'flex', gap: '5px', marginTop: '5px' }}>
                  ...
                  <button 
                    ...
                    className="btn-gov" 
                    style={{ padding: '5px 10px', fontSize: '0.8rem', background: c.assigned_to ? '#6c757d' : '#0056b3' }}
                  >
